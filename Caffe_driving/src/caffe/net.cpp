...
//////////////// by chenyi
extern leveldb::DB* db_tmp=NULL;

template <typename Dtype>
Net<Dtype>::Net(const NetParameter& param, leveldb::DB* db) {
  db_tmp=db;
  Init(param);
}

template <typename Dtype>
void Net<Dtype>::ForwardData() {
  //LOG(ERROR) << "Forwarding " << layer_names_[0];
  layers_[0]->Reshape(bottom_vecs_[0], &top_vecs_[0]);
  layers_[0]->Forward(bottom_vecs_[0], &top_vecs_[0]);
}
//////////////// by chenyi
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  ///////////////// by chenyi
  printf("Total layers in the file: %d \n", num_source_layers);
  fflush(stdout);
  ///////////////// by chenyi 
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    if (target_layer_id == layer_names_.size()) {
      ...
      ///////////////// by chenyi
      LOG(INFO) << "Ignoring source layer " << i << ": " << source_layer_name;
      ///////////////// by chenyi
    ...
    ///////////////// by chenyi
    LOG(INFO) << "Copying source layer " << i << ": " << source_layer_name;
    ///////////////// by chenyi
